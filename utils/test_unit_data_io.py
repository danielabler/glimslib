...
class FenicsImageInterface(TestCase):
    ...
    def test_function_to_image_2d_scalar(self):
        ...
        for i in range(1, n_rep):
            ...
            path_to_img = os.path.join(config.output_dir_testing, 'image_from_function_2d_scalar_%i.nii' % i)
    ...
    def test_function_to_image_2d_vector(self):
        ...
        for i in range(1, n_rep):
            # write to image
            img = dio.create_image_from_fenics_function(fun_list[i - 1], size_new=None)
            path_to_img = os.path.join(config.output_dir_testing, 'image_from_function_2d_vector_%i.nii' % i)
            sitk.WriteImage(img, path_to_img)
            img_list.append(img)
            # read from image
            img_read = sitk.ReadImage(path_to_img)
            ...
            fun_list.append(function)
            # plot function
            path_to_fun_plot = os.path.join(config.output_dir_testing, 'disp_%i.png' % i)
            plott.show_img_seg_f(function=fun_list[i], show=True, path=path_to_fun_plot)
            # compare function with previous one
            self.assertLess(fenics.errornorm(fun_list[i - 1], fun_list[i]),1E-5)
    ...
    def test_function_to_image_3d_scalar(self):
        ...
        for i in range(1, n_rep):
            # write to image
            img = dio.create_image_from_fenics_function(fun_list[i - 1], size_new=None)
            path_to_img = os.path.join(config.output_dir_testing, 'image_from_function_3d_scalar_%i.nii' % i)
            sitk.WriteImage(img, path_to_img)
            img_list.append(img)
            # read from image
            img_read = sitk.ReadImage(path_to_img)
    ...
    def test_function_to_image_3d_vector(self):
        ...
        for i in range(1, n_rep):
            # write to image
            img = dio.create_image_from_fenics_function(fun_list[i - 1], size_new=None)
            path_to_img = os.path.join(config.output_dir_testing, 'image_from_function_3d_vector_%i.nii' % i)
            sitk.WriteImage(img, path_to_img)
            img_list.append(img)
            # read from image
            img_read = sitk.ReadImage(path_to_img)
