...
def generate_path(name='domain', file_type='image', dim=2, file_ext=None, base_dir=None):
    if dim is not None:
        dim_str = "%id" % dim
    else:
        dim_str = ""
    if file_type == 'image' and file_ext is None:
        ext = 'mha'
    elif file_type == 'labels' and file_ext is None:
        ext = 'mha'
    elif file_type == 'trafo' and file_ext is None:
        ext = 'mat'
    else:
        ext = file_ext

    file_name = "%s_%s.%s" % (name, file_type, dim_str, file_ext)
...
class ImageBasedOptimization():
    ...
    def __init__(self, base_dir, path_to_labels=None, path_to_image=None,
        ...
        if (path_to_labels is None) or (path_to_image is None) or (image_z_slice is None):
            if os.path.exists(self.path_to_base_params):
                ...
                self.path_to_labels_atlas = base_params["path_to_labels_atlas"]
                self.path_to_t1_atlas = base_params["path_to_t1_atlas"]
            ...
            self.path_to_labels_atlas = path_to_labels_atlas
            self.path_to_t1_atlas = path_to_image_atlas
            ...
            base_params = {"path_to_labels" : self.path_to_labels,
                           ...
                           "path_to_t1_atlas": self.path_to_t1_atlas,
                           "plot" : self.plot}
    ...
    def plot_all(self):
        # atlas & patient from image
        atlas_labels = sitk.Cast(sitk.ReadImage(self.path_atlas_labels_2d), sitk.sitkUInt8)
        patient_image = sitk.ReadImage(self.path_patient_image_2d)
        patient_labels = sitk.Cast(sitk.ReadImage(self.path_patient_labels_2d), sitk.sitkUInt8)

        # plott.show_img_seg_f(image=patient_image, segmentation=patient_labels,
        #                      path=os.path.join(self.path_domain_prep, 'patient_image_tumor_labels.png'))
        # plott.show_img_seg_f(image=patient_image, segmentation=atlas_labels,
        #                      path=os.path.join(self.path_domain_prep, 'patient_image_atlas_labels.png'))

        # atlas & patient from function
        atlas_labels_fct, _mesh, _subdomains, _boundaries = dio.load_function_mesh(self.path_atlas_labels_function_2d,
                                                                                functionspace='function')
        patient_labels_fct, _mesh, _subdomains, _boundaries = dio.load_function_mesh(self.path_patient_labels_function_2d,
                                                                                functionspace='function')

        # plott.show_img_seg_f(image=patient_image, function=atlas_labels_fct,
        #                      path=os.path.join(self.path_domain_prep, 'patient_image_atlas_labels_from_fct.png'))
        # plott.show_img_seg_f(image=patient_image, function=patient_labels_fct,
        #                      path=os.path.join(self.path_domain_prep, 'patient_image_tumor_labels_from_fct.png'))

        # domain & atlas labels from function, reduced domain
        mesh, subdomains, boundaries = dio.read_mesh_hdf5(self.path_domain_2d_reduced)
        subdomains_fct = vh.convert_meshfunction_to_function(mesh, subdomains)
        # plott.show_img_seg_f(image=patient_image, function=subdomains_fct,
        #                      path=os.path.join(self.path_domain_prep, 'patient_image_sim_domain_red.png'))

        # domain & patient tumor labels in undeformed configuration
        tumor_labels, _mesh, _subdomains, _boundaries = dio.load_function_mesh(self.path_patient_labels_function_2d_ref_frame,
                                                                                functionspace='function')
        plot_obj = {'object':tumor_labels,
                    'alpha' : 0.5}
        # plott.show_img_seg_f(function=subdomains_fct, add_plot_object_post=plot_obj,
        #                      path=os.path.join(self.path_domain_prep, 'sim_domain_with_tumor_labels_ref_frame.png'))

        # domain & conc target fields
        patient_image_ref = sitk.ReadImage(self.path_patient_image_2d_ref_frame)
        target_conc_02, _mesh, _subdomains, _boundaries = dio.load_function_mesh(
                                                                self.path_forward_conc_threshold_02_reduced,
                                                                functionspace='function', degree=2)
        plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels, function=target_conc_02, range_f=[0.001,1.1],
                             showmesh=False, alpha_f=0.8, exclude_min_max=True,
                             path=os.path.join(self.path_domain_prep, 'sim_domain_with_target_conc_02.png'))

        target_conc_08, _mesh, _subdomains, _boundaries = dio.load_function_mesh(
                                                                self.path_forward_conc_threshold_08_reduced,
                                                                functionspace='function', degree=2)
        plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels, function=target_conc_08,range_f=[0.001,1.1],
                             showmesh=False, alpha_f=0.8, exclude_min_max=True,
                             path=os.path.join(self.path_domain_prep, 'sim_domain_with_target_conc_08.png'))

        # domain & displacement fields
        ...
        plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels, function=target_disp,
                             showmesh=False, alpha_f=1, exclude_min_max=True,
                             path=os.path.join(self.path_domain_prep, 'sim_domain_with_target_disp.png'))
        ...
        plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels,
                             function=fenics.project(target_disp_x, target_conc_08.ufl_function_space()),
                             showmesh=False, alpha_f=0.8, exclude_min_max=True,
                             path=os.path.join(self.path_domain_prep, 'sim_domain_with_target_disp_x.png'))
        plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels,
                             function=fenics.project(target_disp_y, target_conc_08.ufl_function_space()),
                             showmesh=False, alpha_f=0.8, exclude_min_max=True,
                             path=os.path.join(self.path_domain_prep, 'sim_domain_with_target_disp_y.png'))
        ...
        if len(seed_position)==2:
            ...
            plott.show_img_seg_f(image=patient_image_ref, segmentation=atlas_labels,
                                 ...
                                 path=os.path.join(self.path_domain_prep, 'sim_domain_with_seed.png'))

    def _init_defaults(self):
        # domain preparation
        self.path_domain_prep = os.path.join(self.base_dir, '00_domain_preparation')
        self.path_3d_atlas_reg_to_patient_image = os.path.join(self.path_domain_prep,'atlas_reg_to_patient_3d_image.nii')
        self.path_3d_atlas_reg_to_patient_seg = os.path.join(self.path_domain_prep,'atlas_reg_to_patient_3d_seg.nii')
        self.path_3d_atlas_reg_to_patient_trafo = os.path.join(self.path_domain_prep, 'atlas_reg_to_patient_3d_trafo.mat')
        # simulation domain (either patient or atlas from patient) 3D
        self.path_domain_image_3d = os.path.join(self.path_domain_prep, 'domain_image_3d.mha')
        self.path_domain_labels_3d = os.path.join(self.path_domain_prep, 'domain_labels_3d.mha')
        # simulation domain (either patient or atlas from patient) 2D
        ...
        # patient 2D
        self.path_patient_image_2d = os.path.join(self.path_domain_prep, 'patient_image_2d.mha')
        self.path_patient_labels_2d = os.path.join(self.path_domain_prep, 'patient_labels_2d.mha')
        self.path_patient_image_function_2d = os.path.join(self.path_domain_prep, 'patient_image_function_2d.h5')
        self.path_patient_labels_function_2d = os.path.join(self.path_domain_prep, 'patient_labels_function_2d.h5')
        self.path_patient_2d = os.path.join(self.path_domain_prep, 'patient_mesh_2d.h5')
        self.path_patient_2d_reduced = os.path.join(self.path_domain_prep, 'patient_mesh_2d_reduced.h5')
        # atlas registered to patient 2D
        self.path_atlas_image_2d = os.path.join(self.path_domain_prep, 'atlas_image_2d.mha')
        self.path_atlas_labels_2d = os.path.join(self.path_domain_prep, 'atlas_labels_2d.mha')
        self.path_atlas_image_function_2d = os.path.join(self.path_domain_prep, 'atlas_image_function_2d.h5')
        self.path_atlas_labels_function_2d = os.path.join(self.path_domain_prep, 'atlas_labels_function_2d.h5')
        self.path_atlas_2d = os.path.join(self.path_domain_prep, 'atlas_mesh_2d.h5')
        self.path_atlas_2d_reduced = os.path.join(self.path_domain_prep, 'atlas_mesh_2d_reduced.h5')
        # patient 2D in reference frame
        self.path_patient_image_2d_ref_frame = os.path.join(self.path_domain_prep, 'patient_image_2d_reference_frame.mha')
        self.path_patient_labels_2d_ref_frame = os.path.join(self.path_domain_prep, 'patient_labels_2d_reference_frame.mha')
        self.path_patient_image_function_2d_ref_frame = os.path.join(self.path_domain_prep, 'patient_image_function_2d_reference_frame.h5')
        self.path_patient_labels_function_2d_ref_frame = os.path.join(self.path_domain_prep, 'patient_labels_function_2d_reference_frame.h5')
        self.path_patient_2d_ref_frame = os.path.join(self.path_domain_prep, 'patient_mesh_2d_reference_frame.h5')
        self.path_patient_2d_reduced_ref_frame = os.path.join(self.path_domain_prep, 'patient_mesh_2d_reduced_reference_frame.h5')
    ...
    def init_forward_problem(self, seed_position, model_params_varying, sim_params,
                             model_params_fixed, path_to_domain=None, seed_from_com=False):
    ...
    def get_seed_from_com(self):
        ...
        conc_sim, mesh_sim, subdomains_sim, boundaries_sim = dio.load_function_mesh(path, functionspace='function', degree=2)
        com = compute_com(conc_sim, fenics.dx)
        return com
    ...
    def create_patient_specific_reference_from_atlas(self):
        # Affine registration to match atlas to patient image
        if os.path.exists(self.path_to_t1_atlas):
            path_to_reference_image = self.path_to_t1
            path_to_moving_image = self.path_to_t1_atlas

            path_to_output_image = '.'.join(self.path_3d_atlas_reg_to_patient_image.split('.')[:-1])
            image_ext = 'nii'
            path_to_affine_transform = self.path_3d_atlas_reg_to_patient_trafo
        ...
        #     self.path_domain_image_3d = self.path_3d_atlas_reg_to_patient_image
        # else:
        #     self.path_domain_image_3d = self.path_to_t1

        if os.path.exists(self.path_to_labels_atlas) and os.path.exists(self.path_3d_atlas_reg_to_patient_trafo):
            path_to_input_image = self.path_to_labels_atlas
            path_to_output_image = self.path_3d_atlas_reg_to_patient_seg

            reg.ants_apply_transforms(path_to_input_image, path_to_reference_image, path_to_output_image,
                                      [self.path_3d_atlas_reg_to_patient_trafo], dim=3, interpolation='GenericLabel')
        #     self.path_domain_labels_3d = self.path_3d_atlas_reg_to_patient_seg
        # else:
        #     self.path_domain_labels_3d = self.path_to_labels



    def extract_2d_domain(self, plot=None, file_type='patient'):
        """
        ...
        if file_type == 'patient':
            path_img_3d    = self.path_to_t1
            path_labels_3d = self.path_to_labels
            path_img_2d    = self.path_patient_image_2d
            path_labels_2d = self.path_patient_labels_2d
            path_img_fct   = self.path_patient_image_function_2d
            path_label_fct = self.path_patient_labels_function_2d
            path_meshfct   = self.path_patient_2d
        elif file_type == 'atlas':
            path_img_3d    = self.path_3d_atlas_reg_to_patient_image
            path_labels_3d = self.path_3d_atlas_reg_to_patient_seg
            path_img_2d    = self.path_atlas_image_2d
            path_labels_2d = self.path_atlas_labels_2d
            path_img_fct   = self.path_atlas_image_function_2d
            path_label_fct = self.path_atlas_labels_function_2d
            path_meshfct   = self.path_atlas_2d
        elif file_type == 'domain':
            path_img_3d    = self.path_3d_atlas_reg_to_patient_image
            path_labels_3d = self.path_3d_atlas_reg_to_patient_seg
            path_img_2d    = self.path_domain_image_2d
            path_labels_2d = self.path_domain_labels_2d
            path_img_fct   = self.path_domain_image_function_2d
            path_label_fct = self.path_domain_labels_function_2d
            path_meshfct   = self.path_domain_2d
        ...
        if plot:
            plott.show_img_seg_f(image=image_select, segmentation=image_label_select, show=True,
                                 path=os.path.join(self.path_domain_prep, file_type, 'label_from_sitk_image.png'))

            plott.show_img_seg_f(function=f_img_label, show=True,
                                 path=os.path.join(self.path_domain_prep, file_type, 'label_from_fenics_function.png'))

            plott.show_img_seg_f(function=f_img, show=True,
                                 path=os.path.join(self.path_domain_prep, file_type, 'from_fenics_function.png'))
        ...
        dio.save_function_mesh(f_img_label, path_label_fct)
    ...
    def reduce_2d_domain(self, file_type='domain'):

        if file_type == 'patient':
            path_meshfct   = self.path_patient_2d
            path_meshfct_red = self.path_patient_2d_reduced
        elif file_type == 'atlas':
            path_meshfct   = self.path_atlas_2d
            path_meshfct_red = self.path_atlas_2d_reduced
        elif file_type == 'domain':
            path_meshfct   = self.path_domain_2d
            path_meshfct_red = self.path_domain_2d_reduced
        elif file_type == 'patient_ref':
            path_meshfct   = self.path_patient_2d_ref_frame
            path_meshfct_red = self.path_patient_2d_reduced_ref_frame

        mesh, subdomains, boundaries = dio.read_mesh_hdf5(path_meshfct)

        # -- reduce domain size
        mesh_thr, subdomains_thr = dio.remove_mesh_subdomain(mesh, subdomains, lower_thr=1, upper_thr=4,
                                                             temp_dir=self.path_target_disp)
        dio.save_mesh_hdf5(mesh_thr, path_meshfct_red, subdomains=subdomains_thr)
    ...
    def estimate_deformation_field_patient_reference(self, plot=None):
        if plot is None:
            plot = self.plot
        #-- registration to obtain displacement field
        path_to_reference_image = self.path_atlas_image_2d
        path_to_deformed_image =  self.path_patient_image_2d

        path_to_warped_image = os.path.join(self.path_target_disp, 'registered_patient_to_atlas')
        path_to_warp_field = os.path.join(self.path_target_disp, 'registered_patient_to_atlas_warp.nii.gz')
        ...
        reg.ants_apply_transforms(input_img=self.path_patient_image_2d, output_file=self.path_patient_image_2d_ref_frame,
                                  reference_img=self.path_atlas_image_2d,
                                  transforms=[path_to_warp_field], dim=2)

        reg.ants_apply_transforms(input_img=self.path_patient_labels_2d, output_file=self.path_patient_labels_2d_ref_frame,
                                  reference_img=self.path_atlas_labels_2d,
                                  transforms=[path_to_warp_field], dim=2,
                                  interpolation='GenericLabel')

        # create fields
        # -- load patient labels
        image_label = sitk.Cast(sitk.ReadImage(self.path_patient_labels_2d_ref_frame), sitk.sitkUInt8)
        f_img_label = dio.image2fct2D(image_label)
        f_img_label.rename("label", "label")

        # -- load patient image
        image = sitk.ReadImage(self.path_patient_image_2d_ref_frame)
        f_img = dio.image2fct2D(image)
        f_img.rename("imgvalue", "label")

        file_type = 'patient_ref'
        # -- plot
        if plot:
        ...
        # -- save label function:
        dio.save_function_mesh(f_img, self.path_patient_image_function_2d_ref_frame)
        dio.save_function_mesh(f_img_label, self.path_patient_labels_function_2d_ref_frame)
        ...
        dio.save_mesh_hdf5(mesh, self.path_patient_2d_ref_frame, subdomains=subdomains.subdomains, boundaries=None)



    def reduce_estimated_deformation_field(self, plot=None):
        if plot is None:
            plot = self.plot
        ...
        dio.save_function_mesh(disp_est_reduced, self.path_forward_disp_reconstructed_reduced,
                                subdomains=subdomains_reduced)
    ...
    def create_thresholded_conc_fields_from_segmentation(self, plot=None, T1_label=5, T2_label=6):
        if plot is None:
            plot = self.plot

        # reduce domain
        f_label_patient, mesh_sim, subdomains_sim, boundaries_sim = dio.load_function_mesh(self.path_patient_labels_function_2d_ref_frame,
                                                                                    functionspace='function', degree=1)

        # -- map to reduced domain
        mesh_reduced, subdomains_reduced, boundaries_reduced = dio.read_mesh_hdf5(self.path_domain_2d_reduced)
        funspace_conc_reduced = fenics.FunctionSpace(mesh_reduced, 'Lagrange', 2)
        f_label_patient_reduced = interpolate_non_matching(f_label_patient, funspace_conc_reduced)

        f_conc = f_label_patient_reduced.copy(deepcopy=True)
        f_conc_np = f_conc.vector().get_local()
        # f_conc_np = float(f_conc_np)
        f_conc_np[np.isclose(f_conc_np, T1_label, 0.1)] = 1  # label 5 -> T1
        f_conc_np[np.isclose(f_conc_np, T2_label, 0.1)] = 0.3  # label 5 -> T1
        np.unique(f_conc_np)
        f_conc.vector().set_local(f_conc_np)

        conc_thresh_02 = fenics.project(thresh(f_conc, 0.2), f_label_patient_reduced.function_space(), annotate=False)
        dio.save_function_mesh(conc_thresh_02, self.path_forward_conc_threshold_02_reduced)
        conc_thresh_08 = fenics.project(thresh(f_conc, 0.8), f_label_patient_reduced.function_space(), annotate=False)
        dio.save_function_mesh(conc_thresh_08, self.path_forward_conc_threshold_08_reduced)
